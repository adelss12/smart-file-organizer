...
export const PYTHON_CLI_SCRIPT = `"""
================================================================================
منظّم الملفات الذكي وكاشف المكررات - نسخة سطر الأوامر (CLI Interactive)
================================================================================
"""
...
def main():
    print("=" * 65)
    print("⚡ أداة تنظيم الملفات وكشف المكررات الذكية")
    print("=" * 65)

    # 1. تحديد المجلد تلقائياً أو توجيهه يدوياً
    default_dir = Path(__file__).resolve().parent
    print(f"المجلد الحالي المقترح: {default_dir}")
    custom_dir = input("اضغط Enter لاعتماد المجلد الحالي، أو الصق مسار المجلد المراد فحصه: ").strip()
    
    if custom_dir:
        src_path = Path(custom_dir).resolve()
    else:
        src_path = default_dir

    if not src_path.exists() or not src_path.is_dir():
        print("[!] خطأ: المسار غير موجود!")
        return

    # 2. طلب اسم المجلد الجديد
    new_folder_name = input("\nأدخل اسم المجلد الجديد للملفات الفريدة (افتراضي: CleanFiles): ").strip()
    if not new_folder_name:
        new_folder_name = "CleanFiles"

    # 3. اختيار طريقة المعالجة: نسخ أم نقل
    print("\\nاختر طريقة المعالجة:")
    print("  [1] نسخ (Copy) - يحافظ على الملفات الأصلية دون مساس (الافتراضي الآمن)")
    print("  [2] نقل (Move) - ينقل الملفات الفريدة ويحذف المكرر لتوفير مساحة القرص فوراً")
    choice = input("خيارك (1 أو 2): ").strip()
    op_mode = "move" if choice == "2" else "copy"

    target_dir = src_path / new_folder_name
    target_dir.mkdir(parents=True, exist_ok=True)
    canonical_target = target_dir.resolve()
    script_file_path = Path(__file__).resolve()

    start_time = time.time()
    print("\\n[1/4] جاري فحص شجرة المجلدات...")

    all_files = []
    # منع الحلقة المفرغة بتعديل dirs[:] لحظياً
    for root, dirs, files in os.walk(src_path):
        current_root = Path(root).resolve()
        dirs[:] = [d for d in dirs if (current_root / d).resolve() != canonical_target]
        ...
        for f in files:
            ...
            if f_path == script_file_path or f_path.is_symlink():
                continue
            if canonical_target in f_path.parents or f_path == canonical_target:
                continue
            try:
                all_files.append((f_path, f_path.stat().st_size))
            except (PermissionError, OSError):
                continue

    total_files = len(all_files)
    total_bytes = sum(s for _, s in all_files)
    print(f"تم العثور على {total_files} ملف ({format_size(total_bytes)})")

    # تسريع الفحص بحسب الحجم
    print("[2/4] تسريع الفحص بمطابقة الأحجام والبصمات الرقمية...")
    size_map = {}
    for f_path, f_size in all_files:
        size_map.setdefault(f_size, []).append(f_path)
    ...
    for f_size, f_list in size_map.items():
        if len(f_list) == 1:
            unique_files.append((f_list[0], f_size))
            continue

        for f_path in f_list:
            f_hash = calculate_full_hash(f_path)
            if not f_hash:
                continue
            if f_hash not in registered_hashes:
                registered_hashes.add(f_hash)
                unique_files.append((f_path, f_size))
            else:
                duplicate_files.append((f_path, f_size))

    # تنفيذ النسخ أو النقل
    print(f"[3/4] جاري {('نقل' if op_mode == 'move' else 'نسخ')} الملفات الفريدة...")
    for src_file, _ in unique_files:
        rel_path = src_file.relative_to(src_path)
        dest_file = target_dir / rel_path

        if dest_file.exists() and dest_file.resolve() != src_file.resolve():
            cnt = 1
            alt = dest_file.parent / f"{dest_file.stem}_v{cnt}{dest_file.suffix}"
            while alt.exists():
                cnt += 1
                alt = dest_file.parent / f"{dest_file.stem}_v{cnt}{dest_file.suffix}"
            dest_file = alt
    ...
    # إذا كان وضع النقل، نحذف المكررات
    if op_mode == "move":
        for dup_path, _ in duplicate_files:
            try:
                if dup_path.exists():
                    dup_path.unlink()
            except Exception:
    ...
    # كتابة التقرير النصي
    print("[4/4] إنشاء التقرير النصي الشامل...")
    report_file = target_dir / "تقرير_تنظيم_الملفات.txt"
    duration = time.time() - start_time
    saved_bytes = sum(s for _, s in duplicate_files)

    with open(report_file, "w", encoding="utf-8") as rep:
        rep.write(f"تقرير تنظيم الملفات\\nالتاريخ: {time.strftime('%Y-%m-%d %H:%M:%S')}\\n")
        rep.write(f"إجمالي الملفات: {total_files}\\n")
        rep.write(f"الملفات الفريدة: {len(unique_files)}\\n")
        rep.write(f"الملفات المكررة المتجاهلة: {len(duplicate_files)}\\n")
        rep.write(f"المساحة الموفرة: {format_size(saved_bytes)}\\n")
        rep.write(f"المستغرق: {duration:.2f} ثانية\\n")

    print("\\n" + "=" * 65)
    print("✨ تمت العملية بنجاح!")
    print(f"الملفات الفريدة في ({target_dir.name}): {len(unique_files)}")
    print(f"المكررات المتجاهلة: {len(duplicate_files)}")
    print(f"المساحة الموفرة: {format_size(saved_bytes)}")
